...
void Track::Init()
{
	...
	judgeLineSprite->SetPosition(GameSystem::GetInstance().GetWindowWidth() / 2, GameSystem::GetInstance().GetWindowHeight() - _judgeLineDeltaPos);
	_backgroundSpriteList.Append(judgeLineSprite);

	/*
	_explosionSprite = new Sprite("explosionspr.csv", false);
	_explosionSprite->SetPosition(GameSystem::GetInstance().GetWindowWidth() / 2, GameSystem::GetInstance().GetWindowHeight() - _judgeLineDeltaPos);

	_failSprite = new Sprite("failjudgespr.csv", false);
	_failSprite->SetPosition(GameSystem::GetInstance().GetWindowWidth() / 2, GameSystem::GetInstance().GetWindowHeight()/2);
	*/
	Sprite* explosionSprite = new Sprite("explosionspr.csv", false);
	explosionSprite->SetPosition(GameSystem::GetInstance().GetWindowWidth() / 2, GameSystem::GetInstance().GetWindowHeight() - _judgeLineDeltaPos);
	...
	for (int noteTick = 0; noteTick < totalTick; )
	{
		...
		Note* note = new Note(sec, _judgeLineDeltaPos);
		note->Init();
